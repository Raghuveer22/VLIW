...
class Node:
    ...
    def print_node(self):
        ...
        for dep in self.WARdeps:
            print(f"  -> {dep.Reg} ({dep.Instruction})")

        print("WAW Dependencies:")
        for dep in self.WAWdeps:
            print(f"  -> {dep.Reg} ({dep.Instruction})")

        print("Children:")
        for child in self.Children:
            print(f"  -> {child.Reg} ({child.Instruction})")
...
for i in range(len(Ins)):
    ...
    N = Node(rname,Instructions[i])# seting instruction but asper code it set sinstruction as a word
    N.add_ins(Ins[i]) #setinh word
...
while len(BegNodeTable)!=0:
    ...
    #deque first element of the node table
    ...
    print(UnitFree,WARCleared,WAWCleared,"Hi",leader.Reg,leader.Instruction,leader.InstructionAsWord)
